...
def crop(png_image_name):
    ...
    # (64, 89, 278, 267)
    x, y, z, a = im.getbbox()
    im2 = im.crop((x - 40, y - 20, z + 40,
                   a + 20))  # show have put im.getbbox() here as an argument, but we get better accuracy when there is a margin around
    print(im2.size)
    # (214, 178)
...
def predict():
    ...
    crop(
        "upload_folder/" + filename)  # crop image to the size of the number. cropping the transparent spaces way from the png increases the accuracy because of better quality
